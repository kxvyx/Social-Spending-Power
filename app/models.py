...
class Group(BaseModel):
    ...
    group_name: str = Field(max_length=30,description="name of group")
    description: str = Field(max_length=60,description="Description of group")
    cost: float = Field(gt=0)
...
class GroupModel(Base):
    ...
    description: Mapped[str] = mapped_column(String(50))
    cost: Mapped[float]
    list_of_bills :Mapped[list[int]] = mapped_column(JSON,default=list)
    created_at: Mapped[date] = mapped_column(Date, server_default=func.current_date())
    updated_at: Mapped[date] = mapped_column(Date, server_default=func.current_date(), onupdate=func.current_date())
